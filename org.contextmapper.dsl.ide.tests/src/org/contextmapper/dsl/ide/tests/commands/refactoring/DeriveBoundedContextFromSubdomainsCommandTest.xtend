...
class DeriveBoundedContextFromSubdomainsCommandTest extends AbstractCMLCommandTest {
	...
	def void testARCommandExecution() {
		...
		// when
		val selectedSubdomains = new JsonArray
		selectedSubdomains.add("TestSubdomain")
		
		val refactoringParams = new JsonArray
		refactoringParams.add("TestBoundedContext")
		refactoringParams.add(selectedSubdomains)
		val result = languageServer.executeCommand(
			new ExecuteCommandParams("cml.ar.deriveBoundedContextFromSDs",
