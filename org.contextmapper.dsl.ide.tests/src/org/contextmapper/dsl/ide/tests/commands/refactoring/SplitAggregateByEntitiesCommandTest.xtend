...
class SplitAggregateByEntitiesCommandTest extends AbstractCMLCommandTest {
	...
	def void testARCommandExecution() {
		...
		val result = languageServer.executeCommand(
			new ExecuteCommandParams("cml.ar.splitAggregateByEntities",
				#[new JsonPrimitive(fileURI), new JsonPrimitive("TestAggregate1")]))
