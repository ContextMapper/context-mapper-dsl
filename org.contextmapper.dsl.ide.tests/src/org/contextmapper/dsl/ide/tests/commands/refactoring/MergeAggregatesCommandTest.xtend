...
class MergeAggregatesCommandTest extends AbstractCMLCommandTest {
	...
	def void testARCommandExecution() {
		...
		refactoringParams.add("TestAggregate1")
		refactoringParams.add("TestAggregate2")
		val result = languageServer.executeCommand(
			new ExecuteCommandParams("cml.ar.mergeAggregates", #[new JsonPrimitive(fileURI), refactoringParams]))
