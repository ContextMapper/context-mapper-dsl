...
class SplitBoundedContextByFeaturesCommandTest extends AbstractCMLCommandTest {
	...
	def void testARCommandExecution() {
		...
		val result = languageServer.executeCommand(
			new ExecuteCommandParams("cml.ar.splitBCByFeature",
				#[new JsonPrimitive(fileURI), new JsonPrimitive("TestContext")]))
