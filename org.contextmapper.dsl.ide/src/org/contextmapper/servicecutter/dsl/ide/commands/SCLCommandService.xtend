...
class SCLCommandService implements IExecutableCommandService {

	static final Logger LOG = Logger.getLogger(SCLCommandService);
	...
	@Inject SCLCommandRegistry commandRegistry
	...
	override execute(ExecuteCommandParams params, ILanguageServerAccess access, CancelIndicator cancelIndicator) {
		...
		if (uri !== null) {
			return access.doRead(uri.asString) [
				LOG.info(
					"SCL LSP command has been called: " + params.getCommand() + " (" + params.getArguments().get(0) +
						")");
				try {
					command.executeCommand(new SCLResourceContainer(resource), document, access, params);
				} catch (Exception e) {
